...
- **Unit (no DB):** `TorznabTests` (caps/query/RSS XML), `CategoryDetectionTests` (`CategoryClassifier.DetectCategory` static), `ConfigurationTests` (config binding + `DatabaseConfiguration` env-var logic), `SnapshotStalenessTests`, `StageTimingLoggerTests` (custom `CapturingLogger`), `StoreResultTests`, `ParserConcurrencyTests`, `ParsingExtractTrailingYearTests`, `MatcherLoggerLevelTests`.
